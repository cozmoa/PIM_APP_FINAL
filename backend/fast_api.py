...
# Around line 280 - REPLACE existing update_note function
@app.put("/notes/{title}")
...
async def get_folders(username: str = Depends(get_current_user)):
    ...
    try:
        ...
        result = json.loads(notes_system.get_user_folders(session_id))
        if result["success"]:
            return create_response(
                success=True,
...
@app.post("/folders")
async def create_folder(folder_data: dict, username: str = Depends(get_current_user)):
    """Create a new folder"""
    try:
        session_id = get_session_id(username)
        if not session_id:
        ...
        result = json.loads(notes_system.create_folder(session_id, name, parent_id))
        if result["success"]:
            return create_response(
                success=True,
